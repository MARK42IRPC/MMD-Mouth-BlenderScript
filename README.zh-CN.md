...
- 只自动查找 MMD 的五个 `あ/い/う/え/お` 形态键。
- 支持直接写入形态键的 `BAKE` 模式，以及控制器属性 `DRIVER` 模式。
- 每个片段独立拥有 Action 和 NLA Strip，并支持安全清理。
- 可将当前模型的所有口型片段烘焙为形态键上的可编辑关键帧控制点，便于时间线编辑和 VMD 形态序列导出。
...
1. 在 Blender 5.2 中安装 `dist-addon/MMDmouth-0.6.2.zip` 插件。
...
8. 需要让当前模型所有片段都显示为可编辑关键帧时，点击 `Bake All Keyframes`（烘焙所有关键帧）。
